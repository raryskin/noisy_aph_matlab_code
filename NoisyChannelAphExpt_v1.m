...
WriteCentered(window, 'Welcome to the experiment. Click the mouse to begin.', Xcenter, Ycenter, black);
Screen('Flip', window);
GetClicks; 

Screen('FillRect',window,white, rect)
instructions1=('-  In this experiment, you will read sentences and answer YES or NO questions.');
...
instructions3=('-  Then, read the question and click on YES or NO to answer.');
WriteLine(window,instructions3, black, 100, 100, Ycenter*.75, 1.25);
Screen('Flip', window,[],1);
GetClicks;

instructions4=('-  Any questions before we get started? If not, click to start.');
...
for trialnum = 1:5; %for testing/demo    
    ...
    accuracy= strcmpi(response,char(randomizedTrials.answer(trialnum)));
